...
<body style="position: relative;">
    <div class="container">
        ...
        @endphp
        <!-- Giao diện -->
        <div class="store-info" style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px; border-bottom: 1px solid #ccc; padding-bottom: 10px;">
            @if($logoBase64)
                <div style="flex-shrink: 0;">
                    <img src="{{ $logoBase64 }}" alt="Logo" style="height: 70px; object-fit: contain;">
                </div>
            @endif
            <div style="flex-grow: 1; text-align: right;">
                <div style="font-size: 14px; font-weight: bold;">{{ $order->cuaHang->ten_cua_hang }}</div>
                <div style="font-size: 13px;">{{ $order->cuaHang->dia_chi }}</div>
                <div style="font-size: 12px; color: #666;">Giờ mở cửa: {{ $order->cuaHang->gio_mo_cua }} - {{ $order->cuaHang->gio_dong_cua }}</div>
            </div>
        </div>
        <!-- Tiêu đề -->
        <div class="header">
        ...
        <div class="order-info">
            ...
            <small>Điểm tích lũy: {{ $order->khachHang->diem_thanh_vien }}</small>
            <small>Hạng: {{ $order->khachHang->hang_thanh_vien }}</small>
            @endif
