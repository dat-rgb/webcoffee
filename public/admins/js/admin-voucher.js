document.addEventListener('DOMContentLoaded', function () {
    ...
    voucherRows.forEach(row => {
        row.addEventListener('click', function(e) {
            ...
            const checkbox = this.querySelector('.voucher-checkbox');
            if (checkbox) {
...
document.addEventListener('DOMContentLoaded', function () {
    const checkAll = document.getElementById('checkAll');
    const checkboxes = document.querySelectorAll('.voucher-checkbox');
    const hideButton = document.getElementById('hide-vouchers');
    const showButton = document.getElementById('show-vouchers');
    const deleteButton = document.getElementById('delete-vouchers');
    const restoreButton = document.getElementById('restore-vouchers');
    const forceDeleteButton = document.getElementById('force-delete-vouchers'); // nút xóa vĩnh viễn
    ...
    function handleBulkButton(button, action, confirmMessage = null) {
        ...
        button.addEventListener('click', () => {
            ...
            if (selectedIds.length === 0) return showWarning('Vui lòng chọn ít nhất 1 Voucher.');
    ...
    handleBulkButton(restoreButton, 'restore', {
        ...
        text: 'Các Voucher đã chọn sẽ được khôi phục!',
    ...
    function performBulkAction(selectedIds, action) {
        ...
        setTimeout(() => {
            $.ajax({
                url: '/admin/vouchers/bulk-action',
                method: 'POST',
                data: {
                    _token: $('meta[name="csrf-token"]').attr('content'),
                    selected_vouchers: selectedIds,
                    action: action
                },
